...
add_assignment_to_course(int id, char* assignment_name, int value){
    ...
    HASH_ITER(hh, myCourse->course_students, myStud, tmpStud){
        struct assignment* myStudAssignment = malloc(sizeof(struct assignment*));
        myStudAssignment = malloc(sizeof(struct assignment*));
        myStudAssignment->name = malloc(strlen(assignment_name));
        memset(myStudAssignment, '\0', strlen(assignment_name));
        myStudAssignment->name = assignment_name;
        myStudAssignment->value = value;
        HASH_ADD_KEYPTR(hh, myStud->student_assignments, myStudAssignment->name, strlen(myStudAssignment->name), myStudAssignment);
    }
...
grade_student(char* studName, char* studPass, int courseID, char* assignmentName, int grade){
    ...
    HASH_ITER(hh, myStud->student_assignments, ass, tmpass){
...
student_handler(int client_sock, struct student* myStud){
    ...
    if(!strcmp("VG", client_message)){
        ...
        struct assignment* ass, *tmpass, *studass, *tmpstudass;
        struct course* myCourse, *tmpcourse;
        char buffer[4096];
        int ret;
        ...
        HASH_ITER(hh, myStud->student_courses, myCourse, tmpcourse){
            ...
            HASH_ITER(hh, myCourse->course_assignments, ass, tmpass){
                HASH_ITER(hh, myStud->student_assignments, studass, tmpstudass){
                    if(!strcmp(ass->name, studass->name)){
                        memset(buffer, '\0', 4096);
                        sprintf(buffer, "\t\t\tName: %s Value: %d Grade: %d\n",studass->name, studass->value, studass->grade);
